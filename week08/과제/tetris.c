...
void recNewRank(int score, double duration, long treeSize, double game_duration){
	// 목적: GameOver시 호출되어 사용자 이름을 입력받고 score와 함께 리스트의 적절한 위치에 저장
	char str[NAMELEN+1];
	int i, j;
	clear();
	...
	// 게임 종료 후 화면
	printw("- play-time: %.2fsec\n", game_duration);
	printw("- time(t): %.2fsec\n", duration);
	printw("- score(t): %d\n", score);
	printw("- space(t): %ldbyte\n", treeSize);

	printw("- time-efficiency(score(t)/time(t)): %.2f\n", (float)score/duration);
	printw("- space-efficiency(score(t)/space(t)): %.2f\n", (float)score/treeSize);
...
void DropToRecommended(int sig){
	...
	// 트리가 구성되는 부분_시공간복잡도 측정
	treeSize = 0;
	start = time(NULL);
	max = recommend(recRoot); //printw(" %d", max);
	stop = time(NULL);
	duration += (double)difftime(stop, start); // 시간누적
...
void recommendedPlay(){
	...
	printw("GameOver!!");
	game_stop = time(NULL);
	game_duration = (double)difftime(game_stop, game_start); // 시간누적
	refresh();
	getch();
	recNewRank(score, duration, treeSize, game_duration);
}
...
void DrawChange(char f[HEIGHT][WIDTH], int command, int currentBlock, int blockRotate, int blockY, int blockX){
	...
	// 5) recommended 블록을 그린다.
	struct RecNode *recRoot;
	recRoot = (struct RecNode *)malloc(sizeof(struct RecNode));
	recRoot->level = 0;
	recRoot->acm_score = 0;
	recRoot->recField = field;
	max = recommend(recRoot);
	DrawBlock(rec_y,rec_x,nextBlock[0],rec_rotation,'R');
}
